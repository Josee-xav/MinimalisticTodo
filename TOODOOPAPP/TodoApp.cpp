...
void TodoApp::deleteTextInTaskTextVec(std::string text_to_delete)
{
	for(int i = 0; i < tasksText.size(); i++)
	{
		if(tasksText.at(i) == text_to_delete)
			tasksText.erase(tasksText.begin() + i);
	}
...
void TodoApp::closeEvent(QCloseEvent* event)
{
	...
	{
		int ret = styledQuestionMessageBox("MTodo | Warning", "Did you save your changes??", "background-color: rgb(24, 24, 24);\ncolor: white;", QMessageBox::Yes | QMessageBox::No | QMessageBox::Close);
...
void TodoApp::deleteTask()
{
	...
	{
		deleteTextInTaskTextVec(senderTask->textBox->toPlainText().toStdString());
